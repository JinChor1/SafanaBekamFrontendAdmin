...
const CompleteBooking = ({bookingId,closeModal,setModalOption}) => {
    ...
    const completeBooking = async () => {
        const payload = {
            beforeBP: beforeBPRecord,
            afterBP: afterBPRecord,
            healthProblem: healthProblem,
            remarks: remarks
        }

        const completeData = await callAPI({
            method: "PATCH",
            apiRoute: `/api/admin/booking/complete/${bookingId}`,
            payload: payload
        }) 
        
        if (completeData) {
            closeModal()
            toast.success(`Booking [${completeData.bookingNumber}] completed successfully!`, {
                position: "top-center",
            })
        }
    }
            ...
            {(!bookingData && isLoading) &&
                <div className='booking-complete-form'>
                    ...
                    <div className="booking-edit-footer">
                        <Skeleton className="button-complete" width={150} height={35}/>
                    </div>
                </div>
            }
            { bookingData && 
                <div className='booking-complete-form'>
                        ...
                        <div className="col-5">
                            ...
                            <div className="booking-complete-section">
                                ...
                                <div className="booking-bp-row">
                                    ...
                                    <input
                                        type="text"
                                        onChange={(e) => setBeforeBPRecord({...beforeBPRecord, SYS: e.target.value})}
                                        value={beforeBPRecord.SYS}
                                        placeholder='SYS'
                                    />
                                    <p>/</p>
                                    <input
                                        type="text"
                                        onChange={(e) => setBeforeBPRecord({...beforeBPRecord, DIA: e.target.value})}
                                        value={beforeBPRecord.DIA}
                                        placeholder='DIA'
                                    />
                                    <p>/</p>
                                    <input
                                        type="text"
                                        onChange={(e) => setBeforeBPRecord({...beforeBPRecord, BPM: e.target.value})}
                                        value={beforeBPRecord.BPM}
                                        placeholder='BPM'
                                    />
                                </div>
                                <div className="booking-bp-row">
                                    ...
                                    <input
                                        type="text"
                                        onChange={(e) => setAfterBPRecord({...afterBPRecord, SYS: e.target.value})}
                                        value={afterBPRecord.SYS}
                                        placeholder='SYS'
                                    />
                                    <p>/</p>
                                    <input
                                        type="text"
                                        onChange={(e) => setAfterBPRecord({...afterBPRecord, DIA: e.target.value})}
                                        value={afterBPRecord.DIA}
                                        placeholder='DIA'
                                    />
                                    <p>/</p>
                                    <input
                                        type="text"
                                        onChange={(e) => setAfterBPRecord({...afterBPRecord, BPM: e.target.value})}
                                        value={afterBPRecord.BPM}
                                        placeholder='BPM'
                                    />
                                </div>
                                <div className="booking-complete-section">
                                ...
                                <div className='row'>
                                    ...
                                    <div className="col-4">
                                        <textarea
                                            ...
                                            placeholder={selectedPart===""?"Please select a part first!":"Describe selected part here..."}
                    ...
                    <div className="booking-edit-footer">
                        <button className="button-complete" onClick={completeBooking}>Complete Booking</button>
                    </div>
                </div>
            }
        </div>
